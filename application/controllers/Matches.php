...
class Matches extends CI_Controller {
    ...
    */
    ...
    public function create_app($app = NULL)
    {
        if(isset($app))
        {
            if(file_exists('application/modules/controllers/'.$this->_filename($app).'.php') OR (class_exists(''.$app.'')) OR (class_exists(''.$app.'_model')))
            {
                // echo $app.' Controller or Model already exists in the application/controllers directory.';
                echo $app.' Controller atau Model sudah ada sebelumnya.';
                // echo 'test atas';
            }
            else
            {
    ...
    public function create_controller()
    {
        ...
        if(isset($controller))
        {
            ...
            if(empty($directories)){
               $directories=strtolower($file_name);
            }
            $dir = 'modules/'.$directories.'/controllers/'.$file_name;
            // echo $dir;
            if(file_exists(APPPATH.'modules/'.$directories.'/controllers/'.$file_name.'.php'))
            {
            //     echo "ouh yeah";
                // echo $this->_ret.$class_name.' Controller already exists in the application/controllers'.$directories.' directory.';
                echo $this->_ret.$class_name.' Controller sudah ada sebelumnya di application/modules/'.$directories.'/controllers.';
            // }else{
            //     echo "gagal";
            }
            else
            {
                ...
                if(file_exists(APPPATH.'modules/'.$directories.'/controllers'))
                {
                    echo 'folder sudah ada';
                }elseif (strlen($directories)>0 && !file_exists(APPPATH.$directories.'/controllers')) {
                    // echo 'belum ada';
                    mkdir(APPPATH.'modules/'.$directories.'/controllers/', 0777, true);
                    echo 'folder berhasil di buat';
                }
                if(write_file(APPPATH.'modules/'.$directories.'/controllers/'.$file_name.'.php',$f))
                {
                    // echo 'gass ke lur';
                    // echo $this->_ret.'Controller '.$class_name.' has been created inside '.APPPATH.'controllers/'.$directories.'.';
                    echo $this->_ret.'Controller '.$class_name.' berhasil di buat di '.APPPATH.'modules/controllers/'.$directories.'.';
                    return TRUE;
                }
                else
                {
                    // echo $this->_ret.'Couldn\'t write Controller.';
                    echo $this->_ret.'Controllers gagal di buat.';
                    return FALSE;
                }
            }
        }
    ...
    */
    public function create_model()
    {
        ...
        if(isset($model))
        {
            ...
            $dir = 'modules/'.$directories.'/models/'.$file_name;
            if(file_exists(APPPATH.'models/'.$file_name.'.php'))
            {
                ...
                echo $this->_ret.$class_name.' Model sudah ada sebelumnya di application/modules/'.$directories.'/controllers.';
            }
            else
            {
                ...
                $f = strtr($f,$this->_find_replace);
                if(file_exists(APPPATH.'modules/'.$directories.'/models'))
                {
                    echo 'folder sudah ada';
                }elseif (strlen($directories)>0 && !file_exists(APPPATH.$directories.'/models')) {
                    // echo 'belum ada';
                    mkdir(APPPATH.'modules/'.$directories.'/models/', 0777, true);
                    echo 'folder berhasil di buat';
                }
                if(write_file(APPPATH.'modules/'.$directories.'/models/'.$file_name.'.php',$f))
                {
                    // echo 'gass ke lur';
                    // echo $this->_ret.'Controller '.$class_name.' has been created inside '.APPPATH.'controllers/'.$directories.'.';
                    echo $this->_ret.'Models '.$class_name.' berhasil di buat di '.APPPATH.'modules/models/'.$directories.'.';
                    return TRUE;
                }
                else
                {
                    // echo $this->_ret.'Couldn\'t write Controller.';
                    echo $this->_ret.'Models gagal di buat.';
                    return FALSE;
                }
            }
        }
    ...
    */
    public function create_view($view = NULL)
    {
        ...
        if(isset($view))
        {
            ...
            $dir = 'modules/'.$directories.'/views/'.$file_name;
            if(file_exists(APPPATH.'views/'.$file_name.'.php'))
            {
                ...
                echo $this->_ret.$class_name.' View sudah ada sebelumnya di application/modules/'.$directories.'/views.';
            }
            else
            {
                ...
                $f = strtr($f,$this->_find_replace);
                if(file_exists(APPPATH.'modules/'.$directories.'/views'))
                {
                    echo 'folder sudah ada';
                }elseif (strlen($directories)>0 && !file_exists(APPPATH.$directories.'/views')) {
                    // echo 'belum ada';
                    mkdir(APPPATH.'modules/'.$directories.'/views/', 0777, true);
                    echo 'folder berhasil di buat';
                }
                if(write_file(APPPATH.'modules/'.$directories.'/views/'.$file_name.'.php',$f))
                {
                    // echo 'gass ke lur';
                    // echo $this->_ret.'Controller '.$class_name.' has been created inside '.APPPATH.'controllers/'.$directories.'.';
                    echo $this->_ret.'View '.$file_name.' berhasil di buat di '.APPPATH.'modules/views/'.$directories.'.';
                    return TRUE;
                }
                else
                {
                    // echo $this->_ret.'Couldn\'t write Controller.';
                    echo $this->_ret.'View gagal di buat.';
                    return FALSE;
                }
            }
        }
